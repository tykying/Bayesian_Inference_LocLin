function ax = pcolor_subplot(grid_vis, contourf_vis_data, subplot_param)
...
for j_x = 1:nPlot_x  % j <-> number of plots along the horizontal direction
    for i_y = 1:nPlot_y
        ...
        % ContourLineValue_data = subplot_param(i_y, j_x).ContourLineValue_data;    % Difference with contourf_subplot.m
        ...
        pcolor(ax{i_y, j_x}, grid_vis.X, grid_vis.Y, contourf_vis_data{i_y, j_x});  % Only difference with contourf_subplot.m
        if isempty(caxis_data) == 0
